...
arrives in L02 or `uart_regs` in L04, the ports each one must expose are already fixed by how
...
yet; each is instantiated here as it is created across L02 through L04, and the system testbench
...
testbench runs at the end of L04.
...
that will answer on that bus arrives in L04; until then the bus exists with nothing behind it.
...
That output then feeds every submodule, and the blocks you write in L02 through L04 all take it the
...
The **TX feeder** is written in L04, once both signals it needs exist. `uart_tx` (L02) sends a byte
when its `start` is pulsed, and the TX FIFO inside `uart_regs` (L04) holds the bytes waiting to go.
...
until `uart_regs` exists in L04 nothing drives the vector, and `to_integer` on an all-`'U'` value
...
defined value rather than `'U'` while the register bank is missing. It is deleted in L04 when
`uart_regs` starts driving that vector for real.

---

### What the testbench pins down
`uart_top_tb` is the system testbench, and it does not run until the last block lands in L04. It
...
and `miso` reach the ATmega328P on the other side of the wire, and the driver built in L05 through
L07 talks to exactly these ports; `rx` and `tx` are the serial line itself. In L07 a board wrapper
...
`uart_tx`, and with them the `BAUD_DIV` conversion, so the top can transmit. L03 adds `sync` on the
`rx` pin, producing `rx_s2`, and `uart_rx` behind it, so the receiver never sees the raw
asynchronous input. L04 adds `uart_regs`, which owns both FIFOs, and the TX feeder that connects it
