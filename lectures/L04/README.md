...
* **The two FIFOs from L03**, one per direction, instantiated by the bank rather than by the top.
...
along with the pulses and levels they expose, already wired into the top. You also need the `fifo`
from L03, which this bank instantiates twice. Read the full [protocol
...
from. The two FIFOs it owns were built in [L03](../L03/appendix/c_fifo.md) and are already in `hw/`.
