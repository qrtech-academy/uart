# L09 - The Real Transport, and Both Toolchains
...
* **`AvrSpi` on top of it**, implementing the same `driver::transport::Interface` seam that L08's
  `driver::transport::Stub` did, only now clocking real bytes. Nothing above the seam changes, which
  is the whole return on the design work in L06.
...
## Prerequisites
This lecture needs the `driver::transport::Interface` seam from L06, which is what the new transport
implements, plus the `Uart` driver from L07 and the host tests from L08, both reused here unchanged.
It also needs the finished `uart_top` from L05, passing its system testbench, because the second
half of the lecture puts that design on the FPGA. From Embedded C it assumes the AVR toolchain,
avr-gcc and avrdude, and register-level programming; from Modern Embedded C++, the `volatile`
semantics of memory-mapped registers. Install **Quartus Prime Lite** before the lecture: it is a
multi-gigabyte download, and it is the one tool the course has not needed until now.
...
## Evaluation
* Which idiom from the host build fails to compile freestanding, and how does L09 replace it?
