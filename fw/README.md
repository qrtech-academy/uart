...
first (L06-L08); only the thin transport at the bottom is AVR-specific (L09). The unit- and
...
```text
include/driver/uart/        The register map, driver::uart::Interface (L06), and this course's
                            driver::uart::Uart (L07).
include/driver/transport/   The transport seam driver::transport::Interface (L06), the scripted
                            driver::transport::Stub used by the host tests (L08), and the real
                            driver::transport::AvrSpi (L09); AvrSpi's source is in
                            source/driver/transport/.
include/app/                The application seam app::Interface, and app::EchoNode (L10).
...
source/                     Implementations: the driver::uart::Uart protocol layer (readReg/writeReg
                            as 5-byte SPI transactions), app::EchoNode (L10), and a provided demo
                            main (L08), plus the driver::transport::AvrSpi source (L09) - the
...
test/                       Host tests (QAcademy Test) exercising the driver over the stub, with no
                            hardware (L08-L10). Each suite is the executable specification of the
                            register protocol.
avr/                        The ATmega328P port: the freestanding C++ runtime stubs (env.cpp), the
                            avr-gcc Makefile, and the main for the EchoNode demo (L09-L10).
...
open is the second one: `test` reports there is nothing to do until the L07-L08 driver headers
...
Three things worth knowing before L09:
* The AVR target is **freestanding**: no C++ standard library at all, no exceptions, no RTTI, no
  ...
  untouched because they were written to that subset from L06; everything is reference-injected into
